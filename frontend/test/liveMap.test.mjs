...
test('LiveDiningMap renders pedestrian agents and density hotspots when provided', () => {
  ...
  assert.equal(mapSource.includes('showAggregateQueueRows'), true)
  assert.equal(mapSource.includes('pedestrianStateLegend'), true)
  assert.equal(mapSource.includes("label: '等同伴/座'"), true)
  assert.equal(mapSource.includes("label: '等待'"), false)
  assert.equal(mapSource.includes('pedestrian-state-legend'), true)
  ...
  assert.equal(styleSource.includes('.pedestrian-state-swatch'), true)
...
test('LiveDiningMap renders the static layout layers without dragging', () => {
  ...
  assert.equal(mapSource.includes('chairStateClass'), true)
  assert.equal(mapSource.includes('is-dining'), true)
  assert.equal(mapSource.includes('is-seat-waiting'), true)
  assert.equal(styleSource.includes('.table-seat.is-dining'), true)
  assert.equal(styleSource.includes('.table-seat.is-seat-waiting'), true)
  assert.equal(mapSource.includes('seatStateLegend'), true)
  assert.equal(mapSource.includes("label: '用餐'"), true)
  assert.equal(mapSource.includes("label: '上座等待'"), true)
  assert.equal(styleSource.includes('.seat-state-legend'), true)
...
test('styles.css declares the live map layers with restrained visuals', () => {
  ...
  assert.equal(styleSource.includes('.pedestrian-state-legend'), true)
  assert.equal(styleSource.includes('.pedestrian-state-swatch'), true)
