...
- 后端统一采用 Python 3.11 + venv + FastAPI + Uvicorn + Pydantic + pytest + pandas，优先完成参数校验、健康检查接口、仿真运行、结果汇总与导出。
...
| pytest | 8.x | 是 | 单元测试与回归检查 | `pytest --version` |
...
由于前端路线已确定为 Vue 3，项目统一使用 Vite 创建 frontend 工程，并安装 Element Plus、ECharts 与 axios。立项阶段至少完成参数输入页、健康检查联通页和结果展示占位页三个基础页面，为后续仿真接入预留接口。
...
HelloWorld 验证的目标不是展示完整功能，而是确认“解释器、依赖、服务启动、浏览器访问和接口返回”五个环节都正常。第 20 组建议先实现一个最小健康检查接口，再在此基础上接入仿真逻辑。
