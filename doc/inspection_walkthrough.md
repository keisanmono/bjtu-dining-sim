...
这份文档用于课程检查现场口播。README 负责当项目总索引，本讲解稿负责按展示顺序把“项目背景、页面操作、前后端调用、仿真核心、指标推荐”串起来。

## 1. 一句话介绍

我们第 20 组做的是北京交通大学就餐仿真系统。系统模拟食堂高峰期学生到达、排队取餐、等待座位、入座就餐和离开的过程，并用指标分析瓶颈，给出窗口、座位和错峰方面的优化推荐。

检查时可以先强调：这是一个前后端分离项目，前端负责配置和可视化，后端负责仿真计算、指标汇总、推荐和数据保存。

## 2. 项目文件结构

| 目录/文件 | 讲解重点 |
|---|---|
| `README.md` | 项目总索引、启动方式、讲解路线和 Mermaid 图 |
...
| `backend/app/main.py` | FastAPI 入口，定义健康检查、参数校验、仿真、推荐、解释、导出接口 |
...
现场演示时建议打开浏览器页面，同时打开 `App.vue`、`api.js`、`main.py`、`simulation.py` 四个文件，对照一次请求链路讲。
...
这个文件统一封装后端接口。讲解时重点说明：前端调用的是 `/api/health`、`/api/config/validate`、`/api/sim/step` 等路径，实际转发由 Vite proxy 完成。

### `frontend/vite.config.js`

这个文件说明前端服务端口是 5173，`/api` 请求转发到 FastAPI 后端 8001。检查时可以用它解释为什么浏览器只访问前端地址，也能调用后端接口。

## 6. 后端关键文件说明

### `backend/app/main.py`

这是 FastAPI 应用入口。讲解重点：
...
这个文件定义接口模型。讲解重点是 `SimulationConfig` 和 `StepRequest`：
...
| 为什么有随机种子？ | 为了让同一配置可以复现实验结果，方便检查和测试。 |
...
| 推荐结果是不是机器学习？ | 不是。这里是候选枚举加规则评分，更适合课程检查时解释原因。 |
| 规则化解释是不是调用大模型？ | 不是。`explanation.py` 是本地规则文本生成，核心功能不依赖 LLM。 |
| 本次检查准备改了功能吗？ | 没有。只扩写 README、增加讲解文档、补充中文讲解注释，不改接口字段、不改算法、不引入依赖。 |
